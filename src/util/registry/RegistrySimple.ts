...
export class RegistrySimple<K, V> implements IRegistry<K, V> {
	...
	public set(key: K, value: V): void {
		Validate.notNull(key, "Parameter 'key' cannot be null or undefined");
		Validate.notNull(value, "Parameter 'value' cannot be null or undefined");

		if (this.keyToValue.has(key)) {
			console.debug(`Adding duplicate key '${key}' to registry`);
		}
